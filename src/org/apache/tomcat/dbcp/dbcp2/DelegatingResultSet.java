...
/**
 * {@link ResultSet}的基础代理实现.
 * <p>
 * {@link ResultSet}接口的所有方法简单调用构造方法中提供的代理的对应方法.
 * <p>
 * 继承 AbandonedTrace 来实现结果集跟踪和记录创建 ResultSet 的代码. 跟踪 ResultSet 确保创建它的 Statement 关闭时, 关闭所有的打开的ResultSet.
 */
public final class DelegatingResultSet extends AbandonedTrace implements ResultSet {
    ...
    /**
     * @param stmt 创建这个ResultSet的Statement
     * @param res 包装的ResultSet
     */
    ...
    /**
     * @param conn 创建这个ResultSet的Connection
     * @param res 包装的ResultSet
     */
    ...
    /**
     * 如果底层的 {@link ResultSet} 不是一个 {@code DelegatingResultSet}, 返回它, 否则以递归方式在委托上调用此方法.
     * <p>
     * 这个方法将返回第一个不是 {@code DelegatingResultSet}的代理,
     * 或 {@code null}, 当遍历这个链找不到非 {@code DelegatingResultSet}的代理时.
     * <p>
     * 如果嵌套了{@code DelegatingResultSet}，并且希望确保获得“真正的”{@link ResultSet}，则此方法很有用.
     */
    ...
    /**
     * 用于关闭ResultSet的包装器, 从跟踪中删除此结果集, 然后在原始ResultSet上调用close.
     */
    @Override
