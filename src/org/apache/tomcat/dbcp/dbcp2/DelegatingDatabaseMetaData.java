...
/**
 * <p>{@link DatabaseMetaData}的基础代理实现.</p>
 *
 * <p>创建{@link ResultSet} 对象的方法被包装来创建{@link DelegatingResultSet} 对象, 其余的方法只是在构造函数中提供的 "delegate" 上调用相应的方法.</p>
 */
public class DelegatingDatabaseMetaData implements DatabaseMetaData {
    ...
    /**
     * 如果底层的 {@link ResultSet} 不是 {@code DelegatingResultSet}, 返回它,
     * 否则在委托上递归调用此方法.
     * <p>
     * 这个方法将返回不是 {@code DelegatingResultSet} 的第一个委托,
     * 或 {@code null}, 当遍历这个链找不到非 {@code DelegatingResultSet}代理时.
     * <p>
     * 如果您可能嵌套了{@code DelegatingResultSet}，并且希望确保获得“真正的”{@link ResultSet}，则此方法很有用.
     * 
     * @return 数据库元数据
     */
