...
public class WsWebSocketContainer implements WebSocketContainer, BackgroundProcess {
    ...
    public Session connectToServer(Object pojo, URI path)
        ...
        // 在使用RI API jar时避免NPE - see BZ 56343
    ...
    private Session connectToServerRecursive(Endpoint endpoint,
        ...
        // 检查是否配置了代理. JavaDoc表示返回值永远不会为 null
        ...
        if (secure) {
            // 不管是否为代理CONNECT创建了非安全包装器, 需要使用TLS从这一点上这样包装原始AsynchronousSocketChannel
            SSLEngine sslEngine = createSSLEngine(userProperties);
            channel = new AsyncChannelWrapperSecure(socketChannel, sslEngine);
        } else if (channel == null) {
            // 只需要包装这个点，如果它没有被包装来处理代理CONNECT
            channel = new AsyncChannelWrapperNonSecure(socketChannel);
        }
        ...
        // WsFrame 添加必要的最终转换. 将完成的转换链复制到远程端点.
        wsRemoteEndpointClient.setTransformation(wsFrameClient.getTransformation());

        endpoint.onOpen(wsSession, clientEndpointConfiguration);
        registerSession(endpoint, wsSession);

        /* 一旦建立了WebSocket 连接，服务器就可能发送一条或多条消息.
         * 根据发送这些消息的确切时间，它们可能位于输入缓冲区中，等待读取，不会触发“可用于读取的数据”事件.
         * 因此, 这里需要处理输入缓冲区. 注意，这发生在当前线程上，这意味着该线程将用于任何onMessage通知.
         * 这是一个特例. 后续的“可读取的数据”事件将由AsyncChannelGroup的执行器线程处理.
         */
    ...
    private HttpResponse processResponse(ByteBuffer response,
        ...
        String line = null;
        while (!readHeaders) {
            // 在进入循环时, 缓冲区将是空的, 并且在新循环开始时, 缓冲器将被完全读取.
            response.clear();
            // 阻塞读取
    ...
    /**
     * 清理由这个容器创建的WebSocket 会话仍在使用的资源. 这包括关闭会话, 并取消与阻塞读/写相关联的{@link Future}.
     */
    public void destroy() {
    ...
    private AsynchronousChannelGroup getAsynchronousChannelGroup() {
        // 使用AsyncChannelGroupUtil 在所有WebSocket 客户端之间共享一个公共组
