...
public class HPackHuffman {
     ...
     * 基于huffman码的树形式的数组.
     * <p/>
     * 如果位被设置, 则高两个字节对应于树节点, 如果它是清空的, 则低两个字节.
     * 如果设置高位, 则它是一个终端节点, 否则它包含下一个节点位置.
     */
    ...
    static {
        ...
        // 实验确定的长度, 将其设置大于它实际结束的大小
        int[] codingTree = new int[256];
        // 树中的当前位置
        int pos = 0;
        int allocated = 1; // 要分配的下一个位置
        // 给定位置的当前状态的map, 只在构建树时使用
    ...
    /**
     * 解码一个 huffman 编码的字符串为目标 StringBuilder. 为了让该方法成功,必须在缓冲区留下足够的空间.
     *
     * @param data   字节缓冲区
     * @param length 要从缓冲区解码的数据的长度
     * @param target 解压缩数据的目标
     *
     * @throws HpackException 如果HPACK header中 Huffman 编码的值, 不是以 EOS 结尾的
     */
    ...
    /**
     * 将给定字符串编码到缓冲区中. 如果缓冲区中没有足够的空间, 或者编码得版本比原始的版本大, 它将返回 false, 并且不修改缓冲区位置.
     *
     * @param buffer   保存数据的缓冲区
     * @param toEncode 要编码的字符串
     * @param forceLowercase 如果字符串应该使用小写编码
     * @return true 如果编码成功
     */
    public static boolean encode(ByteBuffer buffer, String toEncode, boolean forceLowercase) {
        ...
        //这个很烂, 但是需要首先放入长度, 除了提前计算它, 没有别的选择, 确保留下足够的空间, 所以最终迭代了两次
        int length = 0;
