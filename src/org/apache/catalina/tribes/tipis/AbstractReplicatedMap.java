...
//------------------------------------------------------------------------------
    ...
    /**
     * 发送RPC消息通过的 RpcChannel
     */
    protected transient RpcChannel rpcChannel;
    /**
     * 让这个map唯一的 Map 上下文名称, 这允许通过一个channel共享一个以上的map
     */
...
//------------------------------------------------------------------------------
     ...
     * @param mapContextName String - 这个 map的唯一名称, 允许多个 map共用一个 channel
     * @param initialCapacity int - 这个 map的大小, 查看 HashMap
     * @param loadFactor float - 负载因子, see HashMap
     * @param channelSendOptions 发送选项
     * @param cls - 用于反序列化对象的类加载器列表.
     * @param terminate - 是否终止未能启动的 map.
     */
    ...
    /**
     * 通过创建RPC channel初始化 map, 注册它自己为一个 channel 监听器.
     * 该方法还负责启动状态转移
     * @param owner Object
    ...
    protected void init(MapOwner owner, Channel channel, String mapContextName,
        ...
        // 创建一个rpc channel, 并添加 map 作为一个监听器
    ...
    /**
     * 发送一个 ping 给集群中所有的成员, 不只是 map 成员.
     * 
     * @param timeout long
...
//------------------------------------------------------------------------------
    ...
    /**
     * 复制自上次以来的任何修改到对象.
     * 对象必须是初级的, ie, 如果对象是代理或备份, 就不会被复制<br>
     * @param key 要复制的对象
     * @param complete - 如果设置为 true, 对象被复制到它的备份.
     * 如果设置为 false, 只有实现了 ReplicatedMapEntry 的对象, 而且 isDirty() 返回 true, 才会被复制
     */
    public void replicate(Object key, boolean complete) {
        ...
        if (entry.isPrimary() && entry.getBackupNodes()!= null && entry.getBackupNodes().length > 0) {
            //检查是否需要复制这个对象 object isDirty()||complete || isAccessReplicate()
    ...
    /**
     * 这可以由周期性线程调用以复制任何更改.
     * 对于 map, 不会保存实现了 ReplicatedMapEntry 的对象, 这种方法应该经常使用, 以避免大量的数据传输
     ...
     * @return Serializable - null 如果不应该发送回复
     */
    @Override
    public Serializable replyRequest(Serializable msg, final Member sender) {
        ...
        if (mapmsg.getMsgType() == MapMessage.MSG_STATE || mapmsg.getMsgType() == MapMessage.MSG_STATE_COPY) {
            synchronized (stateMutex) { // 确保同一时间不会做两件事情
    ...
    /**
     * 如果答复已被发送到请求线程, rpc 回调可以处理在事实之后出现的任何数据.
     * @param msg Serializable
     * @param sender Member
     */
    @Override
...
//------------------------------------------------------------------------------
    ...
    /**
     * 返回 true, 如果map包含这个 key.
     * 条目可以是代理或备份条目, 执行 <code>get(key)</code>会让这个条目成为这个组的首选.
     * @param key Object
     * @return boolean
     */
    @Override
    ...
    /**
     * 返回map的全部内容.
     * Map.Entry.getValue() 将返回一个包含对象所有信息的 LazyReplicatedMap.MapEntry 对象.
     * @return Set
     */
